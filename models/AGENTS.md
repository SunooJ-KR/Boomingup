...
- 모델 작업을 시작하기 전에 `docs/process.md`를 먼저 읽는다. 어떤 작업이 `Doing`인지, 지금 Phase 게이트가 어디까지 열렸는지 확인하고, 맡을 작업의 상태를 `Doing`으로 바꾼다.
- 작업이 끝나면 `docs/process.md`의 해당 상태를 `Done`으로 바꾸고 산출물과 결정 링크를 함께 갱신한다. 완료 조건을 못 채웠으면 `Done`으로 바꾸지 않고 관련 문서에 남은 일을 적는다.
- 계획 자체를 바꿀 때는 `docs/process.md`와 관련 산출물 문서를 고치고 `docs/decisions.md`에 이유를 적는다. 산식의 정본은 `docs/feature-spec.md`, 후보 범위는 `docs/model-candidates.md`다.
...
- 스크립트 번호는 40번부터 쓴다 (결정 3). `docs/process.md` 계획의 새 스크립트는 그중 60번부터 쓴다
