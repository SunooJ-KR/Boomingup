-- 001~004의 DDL을 적용한 뒤 관리자/readonly 연결에서 실행합니다.
...
with expected_columns(table_name, column_name, data_type) as (
    values
        ...
        ('dong_index', 'snapshot_id', 'bigint'), ('dong_index', 'dong', 'text'), ('dong_index', 'quarter', 'text'), ('dong_index', 'log_index', 'double precision'), ('dong_index', 'n_sales', 'integer'), ('dong_index', 'n_sales_4q', 'integer'), ('dong_index', 'eligible', 'boolean'), ('dong_index', 'log_index_se', 'double precision'),
        ...
        ('dong_support', 'snapshot_id', 'bigint'), ('dong_support', 'dong', 'text'), ('dong_support', 'as_of', 'text'), ('dong_support', 'sale_n_all_4q', 'integer'), ('dong_support', 'n_complexes_4q', 'integer'), ('dong_support', 'dominant_complex_share_4q', 'double precision'), ('dong_support', 'index_se', 'double precision'), ('dong_support', 'index_se_band', 'text'), ('dong_support', 'sample_flags', 'text'), ('dong_support', 'change_12m', 'double precision'), ('dong_support', 'mu_12m', 'double precision'), ('dong_support', 'delta_12m', 'double precision'), ('dong_support', 'delta_se', 'double precision'), ('dong_support', 'delta_state', 'text'), ('dong_support', 'peak_5y_gap', 'double precision'), ('dong_support', 'peak_5y_gap_se', 'double precision'), ('dong_support', 'peak_5y_state', 'text'), ('dong_support', 'structure_type', 'integer'), ('dong_support', 'structure_desc', 'text'), ('dong_support', 'peer_dongs', 'jsonb'),
    ...
    values
        ...
        ('dong_support', 'snapshot_id'), ('dong_support', 'dong'), ('dong_support', 'as_of'),
    ...
    values
        ...
        ('dong_support', 'snapshot_id', 'dataset_snapshot', 'snapshot_id'),
        ('dong_support', 'snapshot_id,dong', 'dong', 'snapshot_id,dong'),
    ...
    -- dong_boundary와 dong_support는 아직 미적용이므로 002·004 DDL의 PostgreSQL 정규형을 추정했습니다.
    values
        ('dong', 'CHECK ((dong = ((sgg_cd || ''_''::text) || umd_nm)))'),
        ('dong_prediction', 'CHECK ((status = ANY (ARRAY[''PREDICTED''::text, ''INSUFFICIENT_SALES''::text])))'),
        ('dong_support', 'CHECK ((index_se_band = ANY (ARRAY[''LOW''::text, ''MID''::text, ''HIGH''::text])))'),
        ('dong_support', 'CHECK ((delta_state = ANY (ARRAY[''DISTINGUISHABLE''::text, ''INDISTINGUISHABLE''::text])))'),
        ('dong_support', 'CHECK ((peak_5y_state = ANY (ARRAY[''AT_PEAK''::text, ''DISTINGUISHABLE''::text, ''INDISTINGUISHABLE''::text])))'),
        ('dong_support', 'CHECK (((structure_type >= 0) AND (structure_type <= 3)))'),
