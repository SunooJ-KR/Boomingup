...
- `app.complex`의 `bjd_code`는 국토부 API `umdCd`와 같은 5자리 법정동 하위코드다. `apt_seq` 앞자리(구 코드)와 함께 쓰면 동이 정해진다(결정 47).
  단지는 매매 기록의 `(sgg_cd, umd_nm)`으로 묶고, 매매가 없는 단지는 `(구 코드, bjd_code)` 사전으로 붙인다. 세대수 기준 커버리지 99.3%.
