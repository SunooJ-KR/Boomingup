...
| `index_se` | `dong_index.log_index_se`, `quarter = T` | 결정 42 정의(표본 잡음 + 수축 편향) |
...
SE 0.032면 12개월 δ 오차(§2.3)가 약 0.045, 2σ 띠가 ±9%다. 2026Q2 동 변화율의 10~90% 폭이 2.8%~22%라 δ 폭도 대략 ±10%다. 즉 이 경계를 넘는 동은 δ가 거의 언제나 "구분되지 않음"이 된다. flag 문장 "작은 변화는 읽지 않는 게 좋아요"가 실제로 맞는 지점이다. 75분위(0.026)로 잡으면 eligible의 25%가 flag를 받아 표시가 흔해진다. 비eligible 100개 동은 최솟값이 0.04라 전부 이 경계 위에 있다.
...
|---|---|---|---|---|---|
| 11140_묵정동 | 20 | 3 | — | 0.0306 | eligible 경계. `FEW_SALES` 없음, `MID` |
| 11710_삼전동 | 19 | 6 | — | 0.0384 | 한 건 차이로 `FEW_SALES` + `HIGH_INDEX_ERROR` |
| 11200_금호동2가 | 80 | 3 | 0.500 | — | 정확히 0.5. flag 없음 |
| 11200_송정동 | 54 | 6 | 0.500 | — | 정확히 0.5. flag 없음 |
| 11140_충무로5가 | 22 | 3 | 0.545 | — | `ONE_COMPLEX_DOMINATES` |
| 11200_금호동1가 | 170 | 5 | 0.541 | — | 매매가 많아도 `ONE_COMPLEX_DOMINATES` |
| 11110_평동 | ≥20 | 1 | 1.000 | — | 단지 하나. `ONE_COMPLEX_DOMINATES` |
| 11170_후암동 | 24 | — | — | 0.0318 | SE 경계 바로 아래. `MID` |
| 11170_서빙고동 | 34 | — | — | 0.0331 | SE 경계 바로 위. `HIGH_INDEX_ERROR` |
...
- `FEW_SALES` 동 100개 전부 `index_se_band == HIGH`다 (비eligible 최소 SE 0.04 > 0.032).
...
| 11140_중림동 | 29 | −0.112 | 0.055 | 2026Q1 | `INDISTINGUISHABLE`. 오차가 커서 수치 안 냄 |
| 11140_흥인동 | 29 | −0.068 | 0.060 | 2021Q4 | `INDISTINGUISHABLE` |
...
- 자동 설명이 예상과 맞는지 4개 전부 수동 확인한다. 예상: 0 "평당가 높음 · 강남 6km대", 1 "평당가 낮음 · 강남 13km대", 2 "신축 비중 높음 · …", 3 "도심 2km대 · …". 실제 문구는 8변수 z로 결정되므로 예상과 다를 수 있다.
...
| 11140_묵정동 (매매 20건, 클러스터 소속에 따라) | 관심 동 eligible 경계 | 후보가 있으면 표시 |
| 클러스터 3의 eligible 47개 | 후보 풀이 가장 작음 | 5개 중 같은 자치구 비율을 첫 실행에서 보고한다 |

같은 자치구 쏠림 사례(5개 중 4개 이상 같은 구)는 행렬이 DB에서만 만들어져 이 문서 작성 시점에 뽑지 못했다. **69 첫 실행이 쏠림 동 수와 예시 3개를 출력하고, 이 표에 추가한다.** 그 전까지 F-4의 경계 사례는 미완이다.
...
## 6. 스크립트 `models/index/69` 요구사항

- 입력: DB(`dong_index`, `dong_feature`, 세대수, 경계 중심점) + `output/62.1` + `output/66.1`. `66`의 `feature_matrix`와 `_dong_weight.dong_households`를 import한다.
