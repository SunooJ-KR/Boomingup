...
| P3-7 | API `queries.ts`를 `dong_support`로 교체, `flows` 집계 추가 | Done | `front/lib/queries.ts`, `types.ts`, `filter.ts`, `format.ts` | P3-6 |
| P3-8 | 화면 컴포넌트 교체 (`payload-schema.md` §7 목록) | Done | `front/components/dong/` | P3-7, P3-5 |
| P3-9 | 샘플 JSON v2 갱신 | Done | `front/public/data/` | P3-2 |
| P3-10 | build 검증 `npx tsc --noEmit && npm run build && npm run check:wording` | Doing | — | P3-8, P3-9 |
