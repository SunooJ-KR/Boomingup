...
| 항목 | 내용 |
|---|---|
| 현재 입력 | `data/tb_etl_tp_admstr_zone_lgldong_bndry/admstr_zone_lgldong_bndry_24.shp`. 서울 법정동 467개와 자치구 코드 25개를 담는다 |
| 출처 확인 | 제공 파일에는 원출처·기준일·이용조건 메타데이터가 없다. `_24`만으로 2024년 자료라고 단정하지 않으며, 운영 배포 전에 제공 기관과 이용조건을 확인한다 |
| 좌표계 | Korea 2000 / Central Belt 2010(EPSG:5186), 인코딩 EUC-KR. `52`가 EPSG:4326 GeoJSON으로 변환 |
| 산출물 | `output/52.1.seoul_bjd_boundary.geojson`(서울 467개 법정동), `front/public/data/dong-boundary.geojson`(웹 단순화 법정동), `front/public/data/gu-boundary.geojson`(25개 자치구 합집합) |
...
| 이전 입력 | GIS Developer의 2023-07 법정동 경계(EPSG:5179)를 사용했으나, 현재 변환 코드는 위 제공 파일의 스키마와 좌표계를 기준으로 한다 |
| 한계 | 법정동 코드·명칭은 이전 산출물 467개와 모두 일치하지만 도형 기준일은 확인되지 않았다. 최신성 판단에는 원출처 메타데이터가 필요하다 |
